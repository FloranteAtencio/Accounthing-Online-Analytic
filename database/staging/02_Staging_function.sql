...
CREATE OR REPLACE FUNCTION Staging.import_validation(
    p_session_id INT,
    p_row_number INT,
...
BEGIN
    ...
    INSERT INTO Staging.import_validation_log(
        session_id
        ,row_number
        ,field_name
        ,validation_rule 
...
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = Finance, Staging, Compliance, Security, Staging, pg_catalog;

-- Function to start an import session
-- This is the very first function needed to call
DROP FUNCTION IF EXISTS Staging.start_import_session(INT, VARCHAR, VARCHAR, VARCHAR) CASCADE;
CREATE FUNCTION Staging.start_import_session(
    p_client_id INT,
...
DECLARE
    v_session_id INT;
BEGIN
    INSERT INTO Staging.import_sessions (client_id, import_type, imported_by, source_file, status)
...
DROP FUNCTION IF EXISTS Staging.log_import_record(INT, INT, VARCHAR, JSONB, VARCHAR, TEXT, INT) CASCADE;
CREATE FUNCTION Staging.log_import_record(
    p_session_id INT,
    p_row_number INT,
...
DECLARE
    v_detail_id BIGINT;
BEGIN
    INSERT INTO Staging.import_detail_logs (
    ...
    UPDATE Staging.import_sessions
...
DROP FUNCTION IF EXISTS Staging.complete_import_session(INT, VARCHAR, TEXT) CASCADE;
CREATE FUNCTION Staging.complete_import_session(
    p_session_id INT,
...
-- Record lineage
DROP FUNCTION IF EXISTS Staging.record_lineage_entry(VARCHAR, INT, INT, VARCHAR, VARCHAR, INT, INT, VARCHAR) CASCADE;
CREATE FUNCTION Staging.record_lineage_entry(
    p_table_name VARCHAR,
    p_record_id INT,
    p_client_id INT,
...
DECLARE
    v_lineage_id BIGINT;
BEGIN
    INSERT INTO Staging.record_lineage (
...
SELECT 'Staging log functions' as STATUS;